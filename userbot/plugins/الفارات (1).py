...
#
# This file is a part of < https://github.com/A1DIIU/ZED_USERBOT/ >
# PLease read the GNU Affero General Public License in
# <https://www.github.com/A1DIIU/ZED_USERBOT/blob/master/LICENSE/>.
...
ZelzalVP_cmd = (
    "𓆩 [𝗦𝗼𝘂𝗿𝗰𝗲 𝐋𝐄𝐓𝐇𝐎𝐍 𝗖𝗼𝗻𝗳𝗶𝗴 𝗩𝗮𝗿𝘀 - اوامـر الفـارات](t.me/A1DIIU) 𓆪\n\n"
    ...
    "\n𓆩 [𐇮 𝙕𝞝𝙇𝙕𝘼𝙇 الهہـيـٖ͡ـ͢ـبـه 𐇮](t.me/S_1_02) 𓆪"
)


# Copyright (C) 2022 A1DIIU . All Rights Reserved
@A1DIIU.on(zelzal_cmd(pattern=r"اضف (.*)"))
@A1DIIU.on(sudo_cmd(pattern=r"اضف (.*)", allow_sudo=True))
async def variable(event):
    ...
    # All Rights Reserved for "A1DIIU - UserBot" "زلـزال الهيبـه"
    if input_str == "كليشة الفحص" or input_str == "كليشه الفحص":
        variable = "ZED_MEDIA"
        await asyncio.sleep(1.5)
        if variable in heroku_var:
            await zed.edit("**✾╎تم تغييـر {} بنجـاح ☑️**\n**✾╎المتغير : ↶**\n `{}` \n**✾╎يتم الان اعـادة تشغيـل بـوت ليثون يستغـرق الامر 2-1 دقيقـه ▬▭ ...**".format(input_str, vinfo))
        else:
...
# Copyright (C) 2022 A1DIIU . All Rights Reserved
@A1DIIU.on(zelzal_cmd(pattern="حذف(?:\s|$)([\s\S]*)"))
@A1DIIU.on(sudo_cmd(pattern="حذف(?:\s|$)([\s\S]*)", allow_sudo=True))
async def variable(event):
    if Config.HEROKU_API_KEY is None:
        return await ed(
            event,
    ...
    if input_str == "كليشة الفحص" or input_str == "كليشه الفحص":
        ...
        await zed.edit("**✾╎تم حـذف {} بنجـاح ☑️**\n**✾╎المتغيـر المحـذوف : ↶**\n `{}` \n**✾╎يتم الان اعـادة تشغيـل بـوت ليثون يستغـرق الامر 2-1 دقيقـه ▬▭ ...**".format(input_str, heroku_var[variable]))
...
# Copyright (C) 2022 A1DIIU . All Rights Reserved
@A1DIIU.on(zelzal_cmd(pattern="جلب(?:\s|$)([\s\S]*)"))
@A1DIIU.on(sudo_cmd(pattern="جلب(?:\s|$)([\s\S]*)", allow_sudo=True))
async def variable(event):
    if Config.HEROKU_API_KEY is None:
        return await ed(
            event,
    ...
    if input_str == "رمز الوقتي" or input_str == "رمز الاسم الوقتي":
        ...
        await asyncio.sleep(1.5)
        if variable in heroku_var:
            await zed.edit("**✾╎الفـار {} موجـود ☑️**\n**✾╎المتغيـر : ↶**\n `{}` \n**✾╎قنـاة السـورس : @A1DIIU**".format(input_str, heroku_var[variable]))
        else:
...
# Copyright (C) 2022 A1DIIU . All Rights Reserved
@A1DIIU.on(zelzal_cmd(pattern="وقت(?:\s|$)([\s\S]*)"))
@A1DIIU.on(sudo_cmd(pattern="وقت(?:\s|$)([\s\S]*)", allow_sudo=True))
async def variable(event):
    if Config.HEROKU_API_KEY is None:
        return await ed(
            event,
    ...
    vmsr = "Africa/Cairo"
    heroku_var = app.config()
    zed = await edit_or_reply(event, "**✾╎جـاري أعـداد المنطقـه الزمنيـه لـ ࢦيــثــوَטּ 🌐...**")
    # All Rights Reserved for "A1DIIU - UserBot" "زلـزال الهيبـه"
    if input_str == "العراق" or input_str == "عراق":
        variable = "TZ"
        await asyncio.sleep(1.5)
        if variable in heroku_var:
            await zed.edit("**✾╎تم تغييـر المنطقـه الزمنيـه .. بنجـاح ☑️**\n**✾╎المتغير : ↶**\n دولـة `{}` 🇮🇶 \n**✾╎يتم الان اعـادة تشغيـل بـوت ليثون يستغـرق الامر 2-1 دقيقـه ▬▭ ...**".format(input_str))
        else:
...
# Copyright (C) 2022 A1DIIU . All Rights Reserved
@A1DIIU.on(admin_cmd(pattern="رفع مطور$", outgoing=True))
@A1DIIU.on(sudo_cmd(pattern="رفع مطور$", allow_sudo=True))
...
# Copyright (C) 2022 A1DIIU . All Rights Reserved
@A1DIIU.on(admin_cmd(pattern="تنزيل مطور$", outgoing=True))
@A1DIIU.on(sudo_cmd(pattern="تنزيل مطور$", allow_sudo=True))
async def zelzalll(event):
    if Config.HEROKU_API_KEY is None:
        return await ed(
            event,
...
@A1DIIU.on(sudo_cmd(pattern="اضف صورة (الحماية|الحمايه|الفحص|الوقتي) ?(.*)", allow_sudo=True))
async def _(tosh):
    if tosh.fwd_from:
        return
    if Config.HEROKU_API_KEY is None:
...
# Copyright (C) 2022 A1DIIU . All Rights Reserved
@A1DIIU.on(zelzal_cmd(pattern="اوامر الفارات"))
@A1DIIU.on(sudo_cmd(pattern="اوامر الفارات", allow_sudo=True))
